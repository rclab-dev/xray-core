...
function xrayRenderTopology(config) {
  ...
  var _holoProto = xray.protocol === "static" ? "Routing Engine" : "OSPF Process";
...
function _xrayRenderTriangleTopology(config) {
  ...
  var _holoProto = _xray.protocol === "static" ? "Routing Engine" : "OSPF Process";
  html += '<div class="xray-holo-panel" id="xray-holo-panel">';
...
function _xrayRenderInvertedVTopology(config) {
  ...
  var _holoProto = _xrayCfg.protocol === "static" ? "Routing Engine" : "OSPF Process";
  html += '<div class="xray-holo-panel" id="xray-holo-panel">';
  html += '<div class="title">[' + _holoProto + " &#8212; " + apex.id + "]</div>";
  html += '<div id="xray-holo-body"></div>';
  html += "</div>";
...
window.xrayBuildRawSummary = function(packets) {
  ...
  if (!packets || packets.length === 0) {
    return "📡 " + routerLabel + " で ICMP は観測されませんでした";
  }
  ...
  var hasReq = focusPkts.some(function(p) {
    return p.raw_line && (p.raw_line.indexOf("request") !== -1 || p.raw_line.indexOf("echo request") !== -1);
  });
  var hasRep = focusPkts.some(function(p) {
    return p.raw_line && (p.raw_line.indexOf("reply") !== -1 || p.raw_line.indexOf("echo reply") !== -1);
  });
  if (hasReq && hasRep) return "📡 " + routerLabel + " で ICMP Echo Request/Reply が観測されました";
  if (hasReq) return "📡 " + routerLabel + " で ICMP Echo Request のみ観測（Reply なし）";
  return "📡 " + routerLabel + " で ICMP パケットが " + focusPkts.length + " 件観測されました";
};
  ...
  var patterns = {
    ...
    ospf_area_mismatch: {
      ...
      destroySteps: [ {
        label: "① あなたのコマンド",
        text: function(s) {
          return "network X.X.X.0/24 area 0";
        },
        engine: "cleared ospf-full",
        arrow: true,
        ping: true,
        panel: function(s) {
          return h.ospfTitle(s._target) + h.areaMatch(s._r2Area || "1", s._r2Area || "1", true) + h.ospfState("Full") + h.ospfRoute(s._target, true) + h.action("FORWARD") + h.ping("0% loss") + h.dim("正常稼働中...");
        }
      }, {
        label: "② Area 不一致",
        text: function(s) {
          return "Area ID を変更 → 不一致";
        },
        engine: "cleared ospf-full",
        arrow: true,
        ping: true,
        panel: function(s) {
          return h.ospfTitle(s._target) + h.areaMatch("0", s._r2Area || "1", false) + h.dim("Area 不一致 → Hello 無視") + h.ospfState("Full") + h.ospfRoute(s._target, true) + h.action("FORWARD") + h.dim("Dead Timer カウント中...");
        }
      }, {
        label: "③ Neighbor 消失",
        text: function(s) {
        ...
        ping: false,
        panel: function(s) {
          return h.ospfTitle(s._target) + h.areaMatch("0", s._r2Area || "1", false) + h.ospfState("None") + h.ospfRoute(s._target, false) + h.action("DROP") + h.ping("100% loss");
        }
      } ]
    },
    ospf_passive: {
